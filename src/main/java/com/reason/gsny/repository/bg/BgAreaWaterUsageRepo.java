...
public interface BgAreaWaterUsageRepo extends JpaRepository<BgAreaWaterUsage,Long> {
     ...
     * @return
     */
    @Query(value = "SELECT new com.reason.gsny.entity.dto.AreaWaterUsage4(sum(au.cost_amount),sum(au.area_water_usage)) from BgAreaWaterUsage au where extract(Year from au.created_at)=extract(Year from CURRENT_TIMESTAMP)")
    List<AreaWaterUsage4> findWaterAndCostForLossRateByYear();
    /**
     * 查询所有的总用水量与总充值量
     * @return
     */
